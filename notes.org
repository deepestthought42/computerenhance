...
#define ACCESSOR(name, type, offset)            \
  type* name(Registers* r) { return (type*)(&r->mem[offset]); };
...
int main() {
    size_t offset = offsetof(Registers, al);
    printf("Offset of member 'ch' is: %zu bytes\n", offset);
...
#+RESULTS:
: Offset of member 'ch' is: 1 bytes
...
void print_bits(i8 num, i8 bits) {
    for (int i = bits - 1; i >= 0; i--) {
        putchar((num & (1 << i)) ? '1' : '0');
    }
    putchar('\n');
